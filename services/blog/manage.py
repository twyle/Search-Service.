...
def seed_db():
    ...
    for article_data in articles_data[:1000]:
    ...
    db.session.commit()
    # es.options(ignore_status=[400,404]).indices.delete(index=os.environ['ES_INDEX'])
    # for count, article in enumerate(articles_data, start=1):
    #     es.index(index=os.environ['ES_INDEX'], document=article, id=str(count))
